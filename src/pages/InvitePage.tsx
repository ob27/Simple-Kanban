...
import { resolveInvite, joinKanban } from '../store';
import type { InviteInfo } from '../store';
...
export function InvitePage() {
  ...
  // Auto-join and redirect once both user and invite are ready
  useEffect(() => {
    if (!user || !invite || hasJoined.current) return;
    hasJoined.current = true;
    joinKanban(invite.kanbanId, user.uid, user.email ?? undefined)
      .catch(() => {}) // already a member or owner — navigate anyway
      .finally(() => navigate(`/k/${invite.kanbanId}`));
  }, [user, invite, navigate]);

  if (resolving || authLoading) return <SpinnerPage />;
  ...
  // User is authenticated — auto-join is in flight via useEffect
  if (user) return <SpinnerPage />;

  // Not authenticated — show branded auth form
      ...
      <div style={{
        ...
        {resetSent ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ fontSize: 13, color: '#888', marginBottom: 20 }}>Password reset link sent. Check your inbox.</div>
