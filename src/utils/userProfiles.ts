...
// (see utils/logoUpload.ts). One-time fetch per uid, cached — nickname edits
// are rare, and every consumer's uid list already changes on its own live
// triggers, so staleness self-heals on the next natural re-render rather
// than needing a dedicated live listener per avatar.
...
// Convenience for the common "nickname-or-email, seed-or-email" display
// pattern used across every member/comment-author rendering site.
export function resolveDisplay(uid: string, email: string, profiles: Record<string, UserProfile>): {
  name: string;
  avatarSeed: string;
  ...
  return {
    name: p?.nickname || email || uid,
    avatarSeed: p?.avatarSeed || email || uid,
